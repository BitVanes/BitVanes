...
type TreeNode =
  ...
  | { kind: 'note'; step: WalkthroughStep };
...
export class StepsTreeProvider implements vscode.TreeDataProvider<TreeNode> {
  ...
  getTreeItem(node: TreeNode): vscode.TreeItem {
    ...
    const item = new vscode.TreeItem(
      `${index + 1}. ${step.title}`,
      step.securityNote ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None,
    );
  ...
  getChildren(element?: TreeNode): TreeNode[] {
    ...
    if (element.kind === 'step') {
      return element.step.securityNote ? [{ kind: 'note' as const, step: element.step }] : [];
    }
    return [];
  }
